...
RUN nix --extra-experimental-features 'nix-command flakes' build \
    ...
    && nix-store --query --requisites /build/toolbox | grep -v -F -x "$(readlink -f /build/toolbox)" > /build/closure \
...
ARG WORKSPACE_RELEASE=0.6.1-preview1
...
RUN ["/workspace-tools/bin/bash", "--noprofile", "--norc", "-c", "printf '%s\\n' \"$WORKSPACE_RELEASE\" > /workspace-tools/manifests/release.txt"]
...
COPY scripts/thin-entry scripts/thin-shell scripts/thin-session scripts/thin-nvim scripts/thin-tmux scripts/thin-agent scripts/thin-editor-tool scripts/thin-app /workspace-tools/
...
ENV PATH="/workspace-tools/bin:/usr/local/bin:/usr/bin:/bin" \
    HOME=/home/workspace LANG=C.UTF-8 \
    LOCALE_ARCHIVE_2_27=/workspace-tools/share/locale-archive \
    WS_LAYOUT=thin-v1 WS_RELEASE=${WORKSPACE_RELEASE}
USER 10001:10001
