...
Inspector remains the source of imported cluster facts. Import an existing local profile through `ws init` or `ws refresh`; the first form preserves those facts and does not rerun Inspector. A later form may expose the same import operation. There is no requirement to select Ruth, Jean, or Blueback on each entry.
