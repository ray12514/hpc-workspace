...
On a stable login host, `ws session --project "$HOME/my-project" --host-jobs` starts the persistent project layout. It requires host tmux and enables the local connection for `ws submit` and `ws jobs` inside the workspace. See the [daily workflow guide](daily-workflow.md) for submission and PuTTY/VS Code appearance settings. Request an interactive allocation from its host-shell window using the site's normal workflow, then run `ws enter --compute --project "$HOME/my-project"` inside that allocation. See [sessions and jobs](../README.md#sessions-and-jobs) for details.
