...
Each image release installs a user-owned snapshot and links it into both agents' skill locations. Existing independent paths are preserved. A new image can update links created by this workspace, while the older snapshot remains available for rollback. Concurrent image releases sharing a home also share these skill links; the most recent entry sets managed links for subsequently started agents.
